...
const auth = (req, res, next) => {
  ...
  jwt.verify(token, SECRET, (err, u) => { if (err) return res.status(403).json({ error: 'Invalid token' }); req.user = u; next(); });
};
...
app.get('/api/dashboard', auth, async (req, res) => {
  ...
  const notScanned = await prisma.animal.findMany({
    where: {
      scans: { none: { scanTimestamp: { gte: new Date(Date.now() - 86400000) } } }
    }
  });
...
app.get('/api/reports/export/excel', auth, async (req, res) => {
  const workbook = new ExcelJS.Workbook();
  const sheet = workbook.addWorksheet('Attendance');
  sheet.columns = [{ header: 'Date', key: 'date' }, { header: 'Animal', key: 'animal' }, { header: 'Staff', key: 'staff' }];
  const scans = await prisma.scan.findMany({ include: { animal: true, user: true } });
  sheet.addRows(scans.map(s => ({ date: s.scanTimestamp, animal: s.animal.name, staff: s.user.name })));
